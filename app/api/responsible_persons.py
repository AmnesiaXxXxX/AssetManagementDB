...
from app.schemas.responsible_person import ResponsiblePersonCreate, ResponsiblePersonUpdate, ResponsiblePersonResponse
...
def get_responsible_person(id: int, db: Session = Depends(get_db)):
    ...
    if not person:
        raise HTTPException(status_code=404, detail="Материально-ответственное лицо не найдено")
    return person

@router.post("", response_model=ResponsiblePersonResponse)
def create_responsible_person(person: ResponsiblePersonCreate, db: Session = Depends(get_db)):
    return service.create(db, person)

@router.put("/{id}", response_model=ResponsiblePersonResponse)
def update_responsible_person(id: int, person: ResponsiblePersonUpdate, db: Session = Depends(get_db)):
